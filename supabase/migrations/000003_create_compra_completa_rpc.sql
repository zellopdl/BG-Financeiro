-- Criacao atomica de compra com itens e parcelas (contas a pagar)
create or replace function public.create_compra_completa(
  p_nf text,
...
begin
  ...
  if p_fornecedor_id is null then
    raise exception 'Fornecedor e obrigatorio';
  end if;

  if p_data is null then
    raise exception 'Data da compra e obrigatoria';
  end if;
  ...
  if v_total <= 0 then
    raise exception 'Total da compra deve ser maior que zero';
  end if;
